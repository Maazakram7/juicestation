  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        <div className="absolute top-0 left-0 right-0 z-30 flex justify-between items-center px-5 md:px-10 pt-10 md:pt-28 text-[9px] md:text-xs uppercase tracking-[0.3em] opacity-50">
          <span>The Process</span>
          <span className="font-mono hidden sm:inline">Fresh &middot; Raw &middot; Cold-pressed</span>
        </div>

        {isMobile ? (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none" aria-hidden="true">
            <div className="w-[120%] h-[70%] opacity-15" style={{ background: 'radial-gradient(circle, #7DC242 0%, transparent 60%)', filter: 'blur(40px)' }} />
          </div>
        ) : (
          <motion.div style={{ rotate: bgRotate }} className="absolute inset-0 flex items-center justify-center pointer-events-none" aria-hidden="true">
            <div className="w-[70%] h-[70%] rounded-full opacity-20 blur-3xl" style={{ background: 'conic-gradient(from 0deg, #7DC242, #F39324, #E94E4E, #7DC242)' }} />
          </motion.div>
        )}
        ...
        <motion.div style={{ x: marqueeX }} className="absolute bottom-8 md:bottom-14 left-0 right-0 z-20 whitespace-nowrap pointer-events-none select-none">
